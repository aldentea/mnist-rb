# ported from torch.rb Examples
# https://github.com/ankane/torch.rb/blob/master/examples/mnist/main.rb
...
def train(model, device, train_loader, optimizer, epoch)
    ...
    train_loader.each_with_index do |(data, target), batch_idx|
        ...
        optimizer.step
        if batch_idx % 2 == 0
            puts "Train Epoch: #{epoch} [#{batch_idx * data.size(0)}/#{train_loader.dataset.size} (#{100.0 * batch_idx / train_loader.size})] Loss: #{loss.item}"
        end
    end
end
...
def test(model, device, test_loader)
    ...
    puts "Test set: Average loss: #{test_loss}, Accuracy: #{correct}/#{test_loader.dataset.size} (#{100.0 * correct / test_loader.dataset.size}%)"
end
